...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (categoryId === 'commercial') {
        ...
        renderHistory(baseContainer, baseData);
        renderHistory(compareContainer, compareData);
    }
...
function renderEducation(c1, c2, d1, d2) {
    ...
    const renderContent = (data) => `
        ...
        <h3 style="margin: 2rem 0 1rem;">學區特色</h3>
        <div style="background:var(--bg-color); padding:1rem 1.5rem; border-radius:8px; border-left:4px solid var(--primary-color);">
            <p style="color:var(--text-primary); line-height:1.6;">${data.features}</p>
        </div>
    `;
    ...
    const renderContent = (data) => `
        ...
        <div style="margin-top: 2rem;">
            <ul class="data-list">
                ...
                <li><strong>總戶數：</strong><span class="counter" data-target="${data.households}">${data.households}</span> 戶</li>
                <li><strong>行政區劃：</strong>${data.villages} 里 / <span class="counter" data-target="${data.neighborhoods}">${data.neighborhoods}</span> 鄰</li>
